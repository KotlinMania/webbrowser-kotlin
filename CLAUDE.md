# Claude Code Project Instructions — webbrowser-kotlin

## Project Overview

This is **webbrowser-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`webbrowser`](https://crates.io/crates/webbrowser).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/webbrowser/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/webbrowser>
